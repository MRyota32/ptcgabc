...
def _evaluate_obs(obs):
    ...
    for p in me.active + me.bench:
        ...
        if p.id == C.MEGA_LUCARIO:
            # エネルギーの価値: 0→0, 1→800, 2→2400(Mega Brave圏は段階的ボーナス)
            energy_val = ec * 800 + (1_600 if ec >= 2 else 0)
            score += 4_000 + energy_val
        elif p.id == C.RIOLU:
            score += 500

    # 自分のactiveのHP: 瀕死なら大幅ペナルティ (3プライズ失うリスク)
    me_a = me.active[0] if me.active else None
    if me_a and me_a.id == C.MEGA_LUCARIO and getattr(me_a, 'maxHp', 0) > 0:
        hp_ratio = me_a.hp / me_a.maxHp
        if hp_ratio <= 0.25:
            score -= 5_000   # 致命的ダメージ: 次の攻撃でKO
        elif hp_ratio <= 0.50:
            score -= 2_000   # 中程度ダメージ
    ...
    if opp_a:
        dmg_dealt = opp_a.maxHp - opp_a.hp
        score += dmg_dealt * 4  # ダメージ蓄積ボーナス
        if opp_a.hp <= 0:
            score += prize_count(opp_a) * 10_000
        elif opp_a.hp <= 130:
            score += 3_000   # Aura Jabでもう1発でKO圏
        elif opp_a.hp <= 270:
            score += 1_500   # Mega Braveで1発KO圏

    # 相手ベンチの脅威: Mega Brave(2エネ)準備済みLucarioは危険
    opp_threats = sum(1 for p in opp.bench
                      if p and p.id == C.MEGA_LUCARIO and len(p.energies) >= 2)
    score -= opp_threats * 2_500
...
class LucarioPolicy:
    ...
    def _energy_score(self, pokemon, is_active) -> float:
        ...
        if pokemon.id == C.MEGA_LUCARIO:
            if ec < 2:
                # Prioritize 0-energy Lucario over 1-energy: faster to Mega Brave
                base = 9000 - ec * 400  # 0 energy→9000, 1 energy→8600
            else:
                base = 200  # already has enough for Mega Brave
        elif pokemon.id == C.RIOLU:
            base = -1  # never attach to Riolu (energy wasted when it evolves)
        else:
            base = 50
    ...
    def _score_retreat(self) -> float:
        ...
        if active.id == C.MEGA_LUCARIO:
            ...
            if ec >= 1:
                # 瀕死(25%HP以下)かつベンチに2エネLucarioあり → 退場して3プライズ守る
                max_hp = getattr(active, 'maxHp', 0)
                if max_hp > 0 and active.hp / max_hp <= 0.25:
                    for p in self.me.bench:
                        if p and p.id == C.MEGA_LUCARIO and self._energy_count(p) >= 2:
                            return 6000  # retreat to preserve prize cards
                return -1  # 通常: 攻撃優先
        # Riolu/その他がactiveなら、エネルギー持ちLucarioと交代
        for p in self.me.bench:
            if p and p.id == C.MEGA_LUCARIO and self._energy_count(p) >= 1:
                return 7000
        return -1
